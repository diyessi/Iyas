...
function train {
    echo "trainQuestionsFocusPath: $trainQuestionsFocusPath"
    echo "trainCasesDir: $trainCasesDir"
    echo "trainOutputDir: $trainOutputDir"
    echo "lang: $lang"
    echo -n "training... "
    java qa.qcri.qf.tools.questionfocus.FocusClassifierTrain -trainQuestionsFocusPath "$trainQuestionsFocusPath" -trainCasesDir "$trainCasesDir" -trainOutputDir "$trainOutputDir" -lang "$lang"
    echo "done"
}
...
function test {
	...
	#echo "lang: $testLang"
	echo "testCasesDir: $testCasesDir"
    echo "testQuestionsFilepath: $testQuestionsFilepath"
    echo "testModelFilepath: $testModelFilepath"
	echo "lang: $lang"
    echo -n "testing... "
    java qa.qcri.qf.tools.questionfocus.FocusClassifierTest -testQuestionsFilepath "$testQuestionsFilepath" -casesDir "$testCasesDir" -testModelFilepath "$testModelFilepath" -lang "$lang"
    echo "done" 
}
...
function cv.train {
    echo "trainQuestionsCVDirectory: $trainQuestionsCVDirectory"
    echo "trainOutputCVDirpath: $trainOutputCVDirpath"
    echo "cvtrainCasesDir: $cvtrainCasesDir"
    echo "lang: $lang"
    ...
    echo -n "cv.training... "
    java qa.qcri.qf.tools.questionfocus.FocusClassifierTrainCV -trainQuestionsCVDirectory "$trainQuestionsCVDirectory" -lang "$lang" -casesDir "$cvtrainCasesDir" -trainOutputCVDirpath "$trainOutputCVDirpath"    
    echo "done"
}
...
function cv.test {
    ...
    echo "testQuestionsCVDirectory: $testQuestionsCVDirectory"
    echo "testModelsCVDirectory: $testModelsCVDirectory"
    echo "cvtestCasesDir: $cvtestCasesDir"
    echo "lang: $lang"
    
    echo -n "cv.testing... "
    java qa.qcri.qf.tools.questionfocus.FocusClassifierTestCV -testQuestionsCVDirectory "$testQuestionsCVDirectory" -testModelsCVDirectory "$testModelsCVDirectory" -casesDir "$cvtestCasesDir" -lang "$lang"
    echo "done"
}
...
usage="usage: bash question_focus.sh [-load config_file] [-train] [-learn] [-test] [-clean [train|test|*]] [-cv.split] [-cv.train] [-cv.learn] [-cv.test] [-cv.clean [train|test|*]]

where:
    -load      load the configuration file
    -train     write the train file
    -learn     train the question focus
    -test      test the question focus
    -clean     remove the serialized CASes
    -cv.split  split dataset into k consecutive folds
    -cv.train  write k different train files
    -cv.learn  train k different question focus
    -cv.test   test  k different question focus
...
while [ -n "$(echo $1 | grep '-')" ]; do 
    case $1 in
        ...
        -help ) echo "-process option help"
                echo "$usage";;
        * ) echo 'usage: bash question_focus.sh [-load config_file] [-train] [-learn] [-test] [-clean] [-cv.split] [-cv.train] [cv.learn] [-cv.test] [-cv.clean]'   
            exit 1
    esac
    shift
done
